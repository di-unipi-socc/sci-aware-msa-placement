    ...
    with PrologMQI() as mqi:
        with mqi.create_thread() as prolog_thread:
            ...
            result = prolog_thread.query(
                "findall(M, microservice(_,_,_), L), length(L, N)."
            )
        return result[0]["N"]
...
class CSVWriter:
        ...
        if not os.path.exists(csvFile):
            ...
            csvwriter.writerow(
                [
                    ...
                    "Time_greenonly",
                    "Time_capacityonly",
                    "Time_linearcombination",
                    "SCI_exhaustive",
                    "SCI_greenonly",
                    "SCI_capacityonly",
                    "SCI_linearcombination",
                    "NumSIZES_exhaustive",
                    "NumSIZES_greenonly",
                    "NumSIZES_capacityonly",
                    "NumSIZES_linearcombination",
                ]
            )
        else:
    ...
    def writeResults(
        self,
        nList,
        resultsListExhaustive,
        resultsListGreenOnly,
        resultsListCapacityOnly,
        resultsListLinearCombination,
    ):
...
class Test:
    ...
    def __init__(self, mode: ModeTest):
        ...
        (
            self._nList,
            self._resultsGreenOnly,
            self._resultsCapacityOnly,
            self._resultsLinearCombination,
            self._resultsExhaustive,
        ) = ([], [], [], [], [])

    def _placementResults(
        self,
        apath: str,
        fpath: str,
        mode: ModeTest,
        ...
        if not failed:
            ...
            if mode == ModeTest.BASE and packing is not None:
                ...
                self._resultsExhaustive.append(
                    {
                        "I": iteration,
                        "Ms": ms,
                        "Time": res["Time"],
        ...
        if res is None or failed:
            results.append(
                {"I": iteration, "Ms": ms, "Time": "X", "SCI": "X", "N": "X"}
            )
            return False
        else:
            res = res.decode("utf-8").strip().replace("'", '"')
    ...
    def _place(self, app: str, infra: str, mode: ModeTest, packing=None):
        ...
        try:
            ...
            print(f"The subprocess timed out after {TIMEOUT} seconds.")
            return None
    ...
    def _getIterationIndex(self, csvFile: str) -> int:
        ...
        if (iterationIndex + NUMBER_OF_RUNS) > len(SEEDS):
            print(
                "The number of NUMBER_OF_RUNS exceeds the length of the array of seeds.\nA random seed will be generated for the next NUMBER_OF_RUNS, proceed? (y/n)"
            )
            while True:
    ...
    def runTests(self, app_name: str):
        if self.__mode == ModeEnv.RANDOM:
            return RandomTestEnv(app_name).runTests()
        elif self.__mode == ModeEnv.CURATED:
            if any(
                node < countMicroservices(full_app_file(app_name)) for node in SIZES
            ):
                print(
                    "Error: A curated enviroment test cannot have less SIZES than the number of microservices in the application."
                )
                sys.exit(1)
            return CuratedTestEnv(app_name).runTests()
        else:
...
class RandomTestEnv(Test):
    ...
    def __init__(self, app_name: str):
        ...
        csvFile = CSV_DIR / f"rnd-{app_name}.csv"
        self.__csvWriter = CSVWriter(csvFile)
        self.__numMS = countMicroservices(full_app_file(app_name))
    ...
    def runTests(self):
        ...
        for iteration in range(
            self.__iterationIndex, self.__iterationIndex + NUMBER_OF_RUNS
        ):
            print(f"Iteration {iteration} starting...")

            # --- Generates the infrastructures specified in SIZES. --- #
            self._deleteTestFiles(INFRA_DIR, re.compile(r"rnd_(\d+)\.pl"))
            SIZESSTR = " ".join(map(str, SIZES))
            try:
                seed = SEEDS[iteration]
            except IndexError:
                seed = random.randint(0, 1000000)
                print(
                    f"Seed for iteration {iteration} was not found. Using random seed: {seed}"
                )
            ...
            for afile in sorted(os.listdir(APP_DIR)):
                apath = os.path.join(APP_DIR, afile)
                ...
                failed = False
                for file in self._getTestFiles(INFRA_DIR, re.compile(r"rnd_(\d+)\.pl")):
                    self._nList.append(self._extractNumber(file))
                    print(
                        f"Executing tests for file: {file}, number of microservices: {ms}..."
                    )
                    fpath = os.path.join(INFRA_DIR, file)

                    # --- Executes the exhaustive solution. --- #
                    if (
                        self._placementResults(
                            apath,
                            fpath,
                            ModeTest.EXHAUSTIVE,
                            self._resultsExhaustive,
...
class CuratedTestEnv(Test):
    ...
    def __init__(self, app_name):
        ...
        csvFile = CSV_DIR / f"curated-{app_name}.csv"
        self.__csvWriter = CSVWriter(csvFile)
        self.__iterationIndex = self._getIterationIndex(csvFile)
        self.__fullApp = full_app_file(app_name)
    ...
    def runTests(self):
        ...
        Iterates through the specified number of NUMBER_OF_RUNS, generates infrastructure, extracts optimal placements,
        ...
        for iteration in range(
            ...
            print(f"Iteration {iteration} starting...")
            SIZESSTR = " ".join(map(str, SIZES))
            self._deleteTestFiles(INFRA_DIR, re.compile(r"crtd_(\d+)\.pl"))
            command = [
                sys.executable,
                "generate.py",
                "--m",
            ...
            for line in sp.stdout.strip().split("\n"):
                try:
                    ...
                    self.__output.append(
                        [f"on({item['Ms']}, {item['N']})" for item in items]
                    )
                except json.JSONDecodeError as e:
                    print(f"Error decoding JSON: {e}")

            # --- Executes the tests for each infrastructure generated. --- #
            for indexFile, file in enumerate(
                self._getTestFiles(INFRA_DIR, re.compile(r"crtd_(\d+)\.pl"))
            ):
                self._nList.append(self._extractNumber(file))
...
def sci_aware(config):
    app_name = config["app_name"]
    mode = config["mode"]
    print(app_name, mode)

    test = Test(ModeEnv[mode.upper()])
    test.runTests(app_name)


if __name__ == "__main__":
    # param_space = {
    #     "app_name": tune.grid_search(["demo", "online-boutique"]),
    #     "mode": tune.grid_search(["random", "curated"]),
    # }
    ...
    # run_config = tune.RunConfig(name=name, storage_path=CSV_DIR)
    # tuner = tune.Tuner(
    #     tune.with_resources(sci_aware, {"cpu": cpus}),
    ...
    sci_aware({"app_name": "demo", "mode": "curated"})
